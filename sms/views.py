...
def teule_home(request):
    ...
    if customer is None:
        ...
        context = {
            ...
            'contacts': Contact.objects.filter(group__customer_id=customer.id).count(),
            'water_clients': TeuleClients.objects.filter().count(),
            'occupied_houses': TeuleHouses.objects.filter(occupied_status__icontains='NO').count(),
            'unoccupied_houses': TeuleHouses.objects.filter(occupied_status__icontains='OCCUPIED').count(),
            'groups': Group.objects.filter(customer_id=customer.id).count(),
            'courts': WaterCourt.objects.filter().count(),
...
def add_meter_readings(request):
    ...
    if request.method == 'POST':
        ...
        standing_charge=sys_configs.standing_charge
        rate=sys_configs.rate
        waterclient = TeuleClients.objects.get(house_number__exact="A14")
        teule_house = TeuleHouses.objects.get(house_number__exact="A14")
