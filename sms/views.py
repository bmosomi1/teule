...
def house_invoices(request, house_id):
    ...
    invoices = TeuleMeterReadings.objects.filter(account_number_id=house_id).order_by('-id')
    ...
    if request.method == 'POST':
        ...
        if customer is not None:
            ...
            for i in range(2, worksheet.max_row + 1):
                ...
                if not TeuleHouses.objects.filter(house_number=house_num).first():
    ...
    if customer is None:
       ...
       # del the_sub_months[0]
        ...
        context = {
            ...
            'groups': Group.objects.filter(customer_id=customer.id).count(),
            'courts': WaterCourt.objects.filter().count(),
            'readings': customer.reading,
            'last_date': customer.read_date,
...
def teule_payments_allocations(request):
    ...
    if request.method == 'POST':
        ...
        context = {
            ...
            'payments_allocated': TeulePaymentReallocate.objects.filter().order_by('-id'),
...
def teule_main_account(request):
    ...
    if customer is None:
        ...
        return render(request, 'sms/teule_main_account.html', context)
...
def teule_home(request):
    ...
    if customer is None:
        ...
        context = {
            ...
            'contacts': Contact.objects.filter(group__customer_id=customer.id).count(),
            'water_clients': TeuleClients.objects.filter().count(),
            'unoccupied_houses': TeuleHouses.objects.filter(occupied_status__icontains='NO').count(),
            'occupied_houses': TeuleHouses.objects.filter(occupied_status__icontains='OCCUPIED').count(),
            'groups': Group.objects.filter(customer_id=customer.id).count(),
            'courts': WaterCourt.objects.filter().count(),
            'readings': TeuleMeterReadings.objects.filter().count(),
            'outbox': TeuleMeterReadings.objects.filter().count(),
            'unallocated_payments': MiwamaMpesa.objects.filter(processed=3).count(),
            'uncollected_rent': int(TeuleHouses.objects.filter(flat_id=1).aggregate(total=Sum('amount_due'))['total'] or 0),
            'unallocated_amount': int(MiwamaMpesa.objects.filter(processed=3).aggregate(total=Sum('amount'))['total'] or 0),
            'admins': CustomerSubAccounts.objects.filter(owner=customer.id).count() + 1
        }
        return render(request, 'sms/teule_apps.html', context)
...
def create_teule_client(request):
    if request.method == 'POST':
        ...
        dear = "Dear "
        your = ", Your  account is "
        flat_names = str(flat_name)
        welcome = " Welcome to "
        your_account_is = ". Your house number is "
        account_num = str(new_cn_int)
        house_num = str(house_number)
        ensure = ". ensure that you put the paybill 4057701 and  account number "
        paying = " when paying for rent and water bill.  Help line 0722812323"
        client_message = dear + client_name + welcome + flat_names + your_account_is + house_number + ensure + house_number + paying
    ...
    context = {
        ...
        'houses': TeuleHouses.objects.filter(occupied_status__icontains='NO')
...
@login_required()
@is_user_customer
def teule_house_dashboard(request,house_id):
    #customer = Customer.objects.filter(user_ptr_id=request.user.id).first()
    customer = TeuleHouses.objects.get(id=house_id)
    if customer is None:
        customer = TeuleHouses.objects.get(id=house_id)
        weeks = get_last_n_weeks(12)
        months = get_last_n_months(12)
       ...
       # del the_sub_months[0]
        ...
        context = {
            'monthly_payments': monthly_payments[::-1],
            'weeks': weeks[::-1],
            'payments': payments,
            'months': the_sub_months[::-1],
            'phone_number': customer.house_number,
        ...
        return render(request, 'sms/teule_house_apps.html', context)
...
def edit_teule_house(request, house_id):
...
def create_teule_flat(request):
    ...
    customer = Customer.objects.filter(id=request.user.id).first()
    if request.method == 'POST':
        caretaker_ids=int(request.POST['caretaker_id'])
        caretaker = Caretaker.objects.filter(id=caretaker_ids).first()
        #phone_number = f"{254}{request.POST['phone_number'].replace(' ', '')[-9:]}"
        TeuleFlat.objects.create(
            caretaker=caretaker,
            customer=customer,
